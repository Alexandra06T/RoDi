...
class TransformerModel(pl.LightningModule):
    def __init__(self, model_name, tokenizer, lr, lr_factor, lr_patience, model_max_length, bio2tags, tag_list):
        ...
        assert self.tokenizer.cls_token_id
        assert self.tokenizer.sep_token_id
...
def run_evaluation(
    ...
    for instance in train_data + validation_data + test_data:
        bio2tags = bio2tags.union(set(zip(instance['ner_ids'], instance['ner_tags'])))
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_name, strip_accents=False)
    ...
    collator = RoNecCollator(tokenizer=tokenizer, max_seq_len=args.model_max_length)
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size, num_workers=1, shuffle=False,
                                collate_fn=collator, pin_memory=True)
    test_dataloader = DataLoader(test_dataset, batch_size=args.batch_size, num_workers=1, shuffle=False,
                                 collate_fn=collator, pin_memory=True)
    ...
    model_checkpointer = pl.callbacks.ModelCheckpoint(save_top_k=1, monitor='valid/strict', dirpath=args.dirpath, filename='{epoch}', mode='max')

    # logger = pl.loggers.wandb.WandbLogger(project='ronec', name=args.name, version=args.version)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--name', type=str, default="ronec")
    parser.add_argument('--version', type=str, default="bert")
    ...
    parser.add_argument('--tokenizer_name', type=str, default="dumitrescustefan/bert-base-romanian-uncased-v1")
    parser.add_argument("--train_file", type=str, default="../datasets/diac/train.json")
    parser.add_argument("--validation_file", type=str, default="../datasets/diac/valid.json")
    parser.add_argument("--test_file", type=str, default="../datasets/diac/test.json")
